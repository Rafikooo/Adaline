    ...
    def fit(self, features, labels):
        ...
        for index in range(self.n_epoch):
            ...
            for i in range(features.shape[0]):
                out = self.output(features[i])
                self.weights[1:] += self.eta * (labels[i][self.classified_digit] - out) * features[i]
...
perceptrons[0].fit(train_images, train_labels)
correct = 0
...
for i, perceptron in enumerate(perceptrons):
    for feature, label in zip(test_images, test_labels):
        ...
        if output == label[0]:
            correct += 1
    print(f"Result on test data, Adaline {i}: {correct}/{n_samples}")
